  ...
  const fetchReviewData = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:3002/reviews/getById",
        {
          id,
        }
      );
      ...
      toast.error(
        "There is some problem in fetching review data. Check your internet connection!!"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async () => {
    if (!localStorage.getItem("user")) {
      ...
      try {
        ...
        const response = await axios.post(
          "http://localhost:3002/reviews/submit",
          {
            id,
  ...
  return (
    <div className="flex flex-col h-full justify-between">
      <div className="w-full min-h-[10rem] pt-2 pd-2 scroll-container">
        {loading ? (
          <FaSpinner className="animate-spin bg-transparent text-3xl mt-auto" />
        ) : reviewList?.length > 0 ? (
